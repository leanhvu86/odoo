...
var SwitchCompanyMenu = Widget.extend({
    ...
    willStart: function () {
        ...
        if(this.allowed_company_ids.length === this.user_companies.length){
            this.allowed_company_ids.push(0);
        }
        this.user_companies.push([0,'All Company']);
    ...
    _onSwitchCompanyClick: function (ev) {
        ...
        if (allowed_company_ids.indexOf(0)>0){
             allowed_company_ids.splice(allowed_company_ids.indexOf(0), 1);
        }
    ...
    _onToggleCompanyClick: function (ev) {
        ...
        if (dropdownItem.find('.fa-square-o').length) {
            if (companyID===0){
                allowed_company_ids=[];
                for (let i =0; i < this.user_companies.length-1;i++){
            ...
            if (companyID===0){
                console.log('companyId = 0',allowed_company_ids)
                allowed_company_ids =[]
            }else{
                console.log('companyId = 1',allowed_company_ids)
