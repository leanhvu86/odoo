...
def pointToNodes(maxCapRoad, costMatrix, node1, node2, vehicle, nodeList):
    check = False

    if costMatrix[node1, node2] > costMatrix[0, node2]:
        return check
...
def nodesToNodes(maxCapRoad, costMatrix, node1, node2, vehicle, nodeList1, nodeList2):
    check = True
    if costMatrix[node1, node2] > costMatrix[0, nodeList2[0]]:
        check = False
        return check
...
def saving(distance, demand, capacity, maxCapRoad, customersList):
    ...
    for i in sortDist:
        ...
        if len(router) == 0:
            ...
            checkMaxCap = False
            if vehicle is not None and vehicle <= maxCapRoad[0, i[0]] and vehicle <= maxCapRoad[i[0], i[1]] and distance[i[0], i[1]] <= distance[0, i[1]]:
                checkMaxCap = True
            if vehicle is not None and vehicle <= maxCapRoad[0, i[1]] and vehicle <= maxCapRoad[i[1], i[0]] and distance[i[1], i[0]] <= distance[0, i[0]]:
                checkMaxCap = True
            ...
            if i[0] in v_i['nodes'] and i[1] not in v_i['nodes']:
                ...
                for k_j, v_j in router.items():
                    ...
                    # kiểm tra các điều kiện về xe, trọng tải trên đường ....
                        checkMaxCap = nodesToNodes(maxCapRoad, distance, i[0], i[1], v_i['Cap'], v_i['nodes'], v_j['nodes'][1:])
                        ...
                        if vehicle is not None:
                            checkMaxCap = nodesToNodes(maxCapRoad, distance, i[0], i[1], vehicle, v_i['nodes'], v_j['nodes'][1:])
            ...
            if i[1] in v_i['nodes'] and i[0] not in v_i['nodes']:
                ...
                for k_j, v_j in router.items():
                    if i[0] in v_j['nodes'] and k_i != k_j:
                        checkConnect = True
                        totalCap = (demand[v_i['nodes']].sum() + demand[v_j['nodes']].sum())
                        checkMaxCap = nodesToNodes(maxCapRoad, distance, i[1], i[0], v_i['Cap'], v_i['nodes'], v_j['nodes'][1:])
                        ...
                        vehicle = getCap(capacity, totalCap)
                        if vehicle is not None:
                            checkMaxCap = nodesToNodes(maxCapRoad, distance, i[1], i[0], vehicle, v_i['nodes'], v_j['nodes'][1:])
        ...
        if not checkConnect and vectorOfPair == 0:
            ...
            checkMaxCap = pointToNodes(maxCapRoad, distance, i[0], i[1], router[keyRoute]['Cap'], router[keyRoute]['nodes'])
            ...
            vehicle = getCap(capacity, totalCap)
            if vehicle is not None:
                checkMaxCap = pointToNodes(maxCapRoad, distance, i[0], i[1], vehicle, router[keyRoute]['nodes'])
        ...
        if not checkConnect and vectorOfPair == 1:
            ...
            checkMaxCap = pointToNodes(maxCapRoad, distance, i[1], i[0], router[keyRoute]['Cap'], router[keyRoute]['nodes'])
            ...
            vehicle = getCap(capacity, totalCap)
            if vehicle is not None:
                checkMaxCap = pointToNodes(maxCapRoad, distance, i[1], i[0], vehicle, router[keyRoute]['nodes'])
            if vehicle is not None and vehicle <= maxCapRoad[i[1], i[0]] and checkMaxCap:
