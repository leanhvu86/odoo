odoo.define('website_ajax_login.website_ajax_login', function (require) {
    ...
    $(document).ready(function () {
        ...
        function custom_popover(element_id, status) {
            if (status == true) {
                element_id.attr("rel", "popover").popover({
                    placement: 'right',
                    html: true,
                    content: '<div id="popOverBox" style="height:50px;width:150px">' + '<button type="button" id="popover_close" class="close  btn-default" aria-label="Close"><span aria-hidden="true">&times;</span></button>' + 'Not justifiable</span></div>',
                });
        ...
        $('#wk_login_button').on('click', function () {
            ...
            $("<div id='wk_ajax_loader'/>").appendTo('body');
            ajax.jsonRpc('/shop/login/', 'call', input).then(function (response) {
                var element = document.getElementById("wk_ajax_loader");
        ...
        $('#wk_signup_button').on('click', function () {
            ...
            $("<div id='wk_ajax_loader'/>").appendTo('body');
                ajax.jsonRpc('/website_ajax_login/signup', 'call', input).then(function (res) {
                var element = document.getElementById("wk_ajax_loader");
                ...
                if (typeof (res['uid']) != "undefined") {
                    ...
                    element.parentNode.removeChild(element);
                    var responses = ["All fields are madatory", "Email is not valid", "This email is already registered", "Password not match", "--Site is under construction please try after sometimes---",];
                    if (list_res.indexOf("filled") > 0) {
                        ...
                        a.each(function (index) {
                            $(this).parent().addClass('has-error has-feedback').append("<span class='fa fa-times form-control-feedback'></span>");
                        });
                    }
        ...
        $('#wk_reset_confirm_button').on('click', function () {
            ...
            if (email.length > 0) {
                ...
                $("<div id='wk_ajax_loader'/>").appendTo('body');
                ajax.jsonRpc('/website_ajax_login/reset_password', 'call', input).then(function (data) {
                    var element = document.getElementById("wk_ajax_loader");
                    element.parentNode.removeChild(element);
                    if (data['error']) {
