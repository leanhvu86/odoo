...
var ActivityMenu = Widget.extend({
    ...
    _onActivityFilterClick: function (event) {
        ...
        if(data.res_model === "fleet.maintenance.schedule"){
            self = this;
            return this._rpc({
                model: data.res_model,
                method: 'get_vehicles_need_maintenance',
                args: [],
                kwargs: {context: session.user_context},
            }).then(function (res) {
                context['search_default_activities_overdue'] = 0
                context['search_default_activities_today'] = 0
                self.do_action({
                    type: 'ir.actions.act_window',
                    name: 'Fleet vehicle',
        ...
        } else if(data.res_model === "sharevan.warehouse"){
            self = this;
            return this._rpc({
                model: data.res_model,
                method: 'get_vehicles_need_maintenance',
                ...
                self.do_action({
                    res_id: res[0],
                    type: 'ir.actions.act_window',
                    name: 'Share van warehouse',
                    res_model: 'sharevan.warehouse',
                    views: [[false, 'form']],
                    search_view_id: [false],
                    context:context,
                });
