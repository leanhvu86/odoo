...
var SidebarFilter = Widget.extend(FieldManagerMixin, {
    ...
    start: function () {
        ...
        this.$el.on('click', '.o_calendar_check_all input', this._onFilterAll.bind(this));
        this.$el.find('.o_calendar_check_all input').prop('checked', true);
        for(var i=0; i < this.filters.length; i++){
            if (!this.filters[i].active){
                this.$el.find('.o_calendar_check_all input').prop('checked', false);
            }
        }
    },
     ...
     * @param {MouseEvent} e
     */
    _onFilterAll: function (e) {
        var $input = $(e.currentTarget);
        console.log('_onFilterAll',$input);
        var listItems = this.$('.o_calendar_filter_items input');
        for(var i=0; i< listItems.length; i++){
            if ($input.prop('checked') != listItems[i].checked){
                listItems[i].checked = $input.prop('checked');
                 this.trigger_up('changeFilter', {
                    'fieldName': this.fieldName,
                    'value': parseInt(listItems[i].closest('.o_calendar_filter_item').dataset.value),
                    'active': $input.prop('checked'),
                });
            }
        }
...
return AbstractRenderer.extend({
    ...
    custom_events: _.extend({}, AbstractRenderer.prototype.custom_events || {}, {
        ...
        deny_event: '_onDenyEvent',
        approve_event: '_onApproveEvent',
    }),
    ...
    getAvatars: function (record, fieldName, imageField) {
        ...
        console.log('getAvatars',record,fieldName,imageField)
        if (!record[fieldName]) {
            return [];
        }
    ...
    _onDenyEvent: function (event){
        this._unselectEvent();
        this.trigger_up('denyRequestCtr', {id: event.data.id});
    },

    _onApproveEvent: function (event){
        this._unselectEvent();
        console.log("Approve request renderer ",event);
        this.trigger_up('approveRequestCtr', {id: event.data.id});
    }
});
