    ...
    *   :class:`AbstractModel` for abstract super classes meant to be shared by
    ...
    def _create_parent_columns(self):
        ...
        if 'parent_path' not in self._fields:
            ...
            _logger.error('parent_path field on model %s must be indexed! Add index=True to the field definition)',
                          self._name)
        ...
        if not fields:
            ...
            if invalid_fields:
                ...
                if not self.env.user.has_group('base.group_no_one'):
                    raise AccessError(
                        _(
                            'You do not have enough rights to access the fields "%(fields)s" on %(document_kind)s (%(document_model)s). ' \
        ...
        for name in fields:
            ...
            for record, vals in data:
                ...
                if not vals:
                    continue
                try:
                    vals[name] = convert(record[name], record, use_name_get, isObject)
                except MissingError:
                    vals.clear()
