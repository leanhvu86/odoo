...
// field types that can be aggregated in grouped views
    ...
    var BasicModel = AbstractModel.extend({
        ...
        _invalidateCache: function (dataPoint) {
            ...
            if (dataPoint.model === 'weight.unit') {
                session.reloadWeightUnit();
            }

             dataPoint.model === 'res.company' && session.reloadAll();


            if (dataPoint.model === 'parcel.unit') {
                session.reloadParcelUnit();
            }
            if (dataPoint.model === 'volume.unit') {
                session.reloadVolumeUnit();
            }
