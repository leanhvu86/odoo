odoo.define('web.Notification', function (require) {
    ...
    var Notification = Widget.extend({
         ...
         */
        start: function () {
            if (this.message==='load_map'){
                utils.set_cookie('load_routing_map', '1');
                utils.set_cookie('reload_load_routing_map', '1');
                console.log('load_routing_map',this.message);
                this.close(true);
            }
            if (this.message==='load_map_success'){
                utils.set_cookie('load_routing_map', '0');
                utils.set_cookie('reload_load_routing_map', '2');
                console.log('load_routing_map',this.message);
                this.close(true);
            }
            if (this.title =="notifications") {
                var current_count = $(".o_notification_count")[0].textContent;
                var tag_current_count = $(".o_notification_count")
                if (current_count == "0") {
                    $(".o_notification_count").removeAttr("style");
                    tag_current_count.text(Number(current_count) + 1);
                } else {
                    tag_current_count.text(Number(current_count) + 1);
                }
            }
