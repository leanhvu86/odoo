...
class XLSXExport(models.AbstractModel):
    ...
    @api.model
    def get_eval_context(self, record, value):
        eval_context = {
    ...
    def _get_line_vals(self, record, line_field, fields):
        ...
        # chèn data vào đây
        lines = json.loads(record.replace("\'", "\""))
    ...
    @api.model
    def _eval_style_cond(self,record, value, style_cond):
        eval_context = self.get_eval_context(record, value)
        ...
        for line_field in line_fields:
            ...
            for rc, field in ws.get(line_field, {}).items():
                ...
                for (row_val, style) in vals[field]:
                    ...
                    # lst = '\"Dog,Cat2,Bat1\"'
                    # data_val = DataValidation(type="list", formula1=lst, allow_blank=True)
                    # st.add_data_validation(data_val)
                    # data_val.add(st['A5'])
                    if row_val not in ("None", None):
                        st[new_rc] = co.str_to_number(row_val)
                    if style:
                        styles = self.env["xlsx.styles"].get_openpyxl_styles()
                        co.fill_cell_style(st[new_rc], style, styles, row_val)
                    i += 1
    ...
    def export_xlsx(self, template, res_model, res_id):
        ...
        ptemp = config['data_dir']  #ConfParam.get_param("path_temp_file") or "/tmp"
        ...
        self._fill_workbook_data(wb, record['results'], export_dict)
        # Return file as .xlsx
        content = BytesIO()
        wb.save(content)
    ...
    @api.model
    def export_xlsx_by_template_and_data(self, template,data):
        data_dict = co.literal_eval(template.instruction.strip())
        export_dict = data_dict.get("__EXPORT__", False)
        out_name = template.name
        if not export_dict:  # If there is not __EXPORT__ formula, just export
        ...
        os.remove(ftemp)
        # Start working with workbook
        self._fill_workbook_data(wb, data, export_dict)
        # Return file as .xlsx
        content = BytesIO()
        wb.save(content)
        content.seek(0)  # Set index to 0, and start reading
