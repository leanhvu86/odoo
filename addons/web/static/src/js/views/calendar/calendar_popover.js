odoo.define('web.CalendarPopover', function (require) {
    ...
    var CalendarPopover = Widget.extend(StandaloneFieldManagerMixin, {
        ...
        events: {
            ...
            'click .o_cw_popover_deny': '_onClickPopoverDeny',
            'click .o_cw_popover_approve': '_onClickPopoverApprove'
        },
        /**
         * @constructor
         * @param {Widget} parent
        ...
        start: function () {
            ...
            self.$('.o_cw_popover_fields_secondary');
            console.log("Calendar popup");
            if (this.modelName == 'fleet.request.time.off') {
                self.$('.o_cw_popover_edit').css("display", "none");
                self.$('.o_cw_popover_delete').css("display", "none");
            }

            console.log("request_day", this.event.record.request_day);
            if (this.modelName != 'fleet.request.time.off' || this.event.record.status != 1
                || !this.event.record.can_approve || this.event.record.request_day <= new Date()) {
                self.$('.o_cw_popover_deny').css("display", "none");
                self.$('.o_cw_popover_approve').css("display", "none");
            }
        ...
        _onClickPopoverDeny: function (ev) {
            ev.preventDefault();
            this.trigger_up('deny_event', {id: this.event.id});
        },
        _onClickPopoverApprove: function (ev) {
            ev.preventDefault();
            this.trigger_up('approve_event', {id: this.event.id});
        },
    });
