odoo.define('web.basic_fields', function (require) {
    ...
    var FieldMonetary = NumericField.extend({
        ...
        _setCurrency: function () {
            ...
            this.currency = session.get_currency(session.currency_id);
            this.formatOptions.currency = this.currency; // _formatValue() uses formatOptions
        },
    });

    var FieldWeightUnit = NumericField.extend({
        description: _lt("WeightUnit"),
        className: 'o_field_monetary o_field_number',
        tagName: 'span',
        supportedFieldTypes: ['float', 'monetary'],
        resetOnAnyFieldChange: true, // Have to listen to currency changes
        init: function () {
            ...
            this.formatOptions.weight_unit = this.weight_unit;
            this.formatOptions.digits = [16, 2];
            this.formatOptions.field_digits = this.nodeOptions.field_digits;
        },
        isSet: function () {
        ...
        _renderEdit: function () {
            ...
            var def = this._prepareInput(this.$input).appendTo(this.$el);
            if (this.weight_unit) {
                var $weight_unit = $('<span>', {text: this.formatOptions.weight_unit});
                if (this.formatOptions.position === "after") {
                    this.$el.append($weight_unit);
                } else {
                    this.$el.prepend($weight_unit);
                }
            }
            return def;
        },
        ...
        _setWeightUnit: function () {
            this.weight_unit = session.weight_unit;
            this.formatOptions.weight_unit = this.weight_unit;
            this.formatOptions.position = 'after';
        },
    });

    var FieldVolumeUnit = NumericField.extend({
        description: _lt("VolumeUnit"),
        className: 'o_field_monetary o_field_number',
        tagName: 'span',
        supportedFieldTypes: ['float', 'monetary'],
        resetOnAnyFieldChange: true, // Have to listen to currency changes
        init: function () {
            ...
            this.formatOptions.volume_unit = this.volume_unit;
            this.formatOptions.digits = [16, 2];
            this.formatOptions.field_digits = this.nodeOptions.field_digits;
        },
        isSet: function () {
        ...
        _renderEdit: function () {
            ...
            var def = this._prepareInput(this.$input).appendTo(this.$el);
            if (this.volume_unit) {
                var $volume_unit = $('<span>', {text: this.formatOptions.volume_unit});
                if (this.formatOptions.position === "after") {
                    this.$el.append($volume_unit);
                } else {
                    this.$el.prepend($volume_unit);
                }
            }
            return def;
        },
        ...
        _setVolumeUnit: function () {
            this.volume_unit = session.volume_unit;
            this.formatOptions.volume_unit = this.volume_unit;
            this.formatOptions.position = 'after';
        },
    });

    var FieldLengthUnit = NumericField.extend({
        description: _lt("LengthUnit"),
        className: 'o_field_monetary o_field_number',
        tagName: 'span',
        supportedFieldTypes: ['float', 'monetary'],
        resetOnAnyFieldChange: true, // Have to listen to currency changes
        init: function () {
            ...
            this.formatOptions.length_unit = this.length_unit;
            this.formatOptions.digits = [16, 2];
            this.formatOptions.field_digits = this.nodeOptions.field_digits;
        },
        isSet: function () {
        ...
        _renderEdit: function () {
            ...
            var def = this._prepareInput(this.$input).appendTo(this.$el);
            if (this.length_unit) {
                var length_unit = $('<span>', {text: this.formatOptions.length_unit});
                if (this.formatOptions.position === "after") {
                    this.$el.append(length_unit);
                } else {
                    this.$el.prepend(length_unit);
                }
            }
            return def;
        },
        ...
        _setLengthUnit: function () {
            this.length_unit = session.length_unit;
            this.formatOptions.length_unit = this.length_unit;
            this.formatOptions.position = 'after';
        },
    });
    ...
    var FieldParcelUnit = NumericField.extend({
        description: _lt("WeightUnit"),
        className: 'o_field_monetary o_field_number',
        tagName: 'span',
        supportedFieldTypes: ['float', 'monetary'],
        ...
        init: function () {
            ...
            this.formatOptions.parcel_unit = this.parcel_unit;
            this.formatOptions.digits = [16, 2];
            this.formatOptions.field_digits = this.nodeOptions.field_digits;
        },
        isSet: function () {
        ...
        _renderEdit: function () {
            ...
            var def = this._prepareInput(this.$input).appendTo(this.$el);
            if (this.parcel_unit) {
                var $parcel_unit = $('<span>', {text: this.formatOptions.parcel_unit});
                if (this.formatOptions.position === "after") {
                    this.$el.append($parcel_unit);
                } else {
                    this.$el.prepend($parcel_unit);
                }
            }
            return def;
        },
        ...
        _setParcelUnit: function () {
            this.parcel_unit = session.parcel_unit;
            this.formatOptions.parcel_unit = this.parcel_unit;
            this.formatOptions.position = 'after';
        },
    });
    ...
    var AbstractFieldBinary = AbstractField.extend({
        ...
        init: function (parent, name, record) {
            ...
            this.max_upload_size = 5 * 1024 * 1024; // 5Mb
            if (!this.useFileAPI) {
                var self = this;
        ...
        on_file_uploaded: function (size, name) {
            if (!name.match(/.(bmp|gif|jpeg|jpg|png|po|tiff|webp)$/i)) {
                this.do_warn(_t("File Upload"), _t("Not an image"))
    ...
    var FieldBinaryImage = AbstractFieldBinary.extend({
        ...
        _render: function () {
            ...
            $img.one('error', function () {
                ...
                // self.do_warn(_t("Image"), _t("Could not display the selected image.")); cmt: vehicle error
            });
    ...
    var FieldBinaryImageCustom = AbstractFieldBinary.extend({
        description: _lt("Image"),
        fieldDependencies: _.extend({}, AbstractFieldBinary.prototype.fieldDependencies, {
            __last_update: {type: 'datetime'},
        }),
    ...
    var RatingStartWidget = AbstractField.extend({
        ...
        _render: function () {
            ...
            return self._rpc({
                ...
                _.each([1, 2, 3, 4, 5], function (index) {
                    if (data >= index) {
                        rating_str += "<span class=\"star on\"></span>\n";
                        check += 1
                        minus += 1
                    } else if (index == check && data + check - 1 > index) {
                        rating_str += " <span class=\"star half\"></span>\n";
                        odd = data + check - minus - index;
                    } else {
                        rating_str += "<span class=\"star\"></span>\n";
                    }
                });

                self.$el.append("<div class=\"stars\"> " +
                    rating_str +
                    "\n" +
                    "</div>");
                ...
                if (odd <= 0.25 && odd > 0) {
                    width = "4"
                } else if (odd > 0.25 && odd <= 0.5) {
                    width = "7"
                } else if (odd > 0.5 && odd <= 0.75) {
                    width = "10"
                } else if (odd > 0.75 && odd <= 1) {
                    width = "12"
                }
    ...
    return {
        ...
        FieldBinaryImageCustom: FieldBinaryImageCustom,
        ...
        FieldWeightUnit: FieldWeightUnit,
        FieldVolumeUnit: FieldVolumeUnit,
        FieldLengthUnit: FieldLengthUnit,
        FieldParcelUnit: FieldParcelUnit,
        ...
        RatingStartWidget: RatingStartWidget,
        StatInfo: StatInfo,
        UrlWidget: UrlWidget,
