...
var CalendarController = AbstractController.extend({
    custom_events: _.extend({}, AbstractController.prototype.custom_events, {
        ...
        denyRequestCtr: '_onDenyEvent',
        approveRequestCtr: '_onApproveEvent',
    }),
    /**
     * @override
     * @param {Widget} parent
    ...
    _onDenyEvent: function (event) {
        ...
        console.log("Deny request controller",event_data);
        return self._rpc({
            model: 'fleet.group.request.to',
            method: 'deny_from_calendar',
            args: [event_data.id],
    ...
    _onApproveEvent: function (event) {
        var self = this;
        var event_data = event.data;
        console.log("Accept request controller",event_data);
        return self._rpc({
            model: 'fleet.group.request.to',
            method: 'approve_from_calendar',
            args: [event_data.id],
            kwargs: {context: session.user_context},
        }).then(function (result) {
            self.reload();
