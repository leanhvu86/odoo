...
from .tools import ustr, consteq, frozendict, pycompat, unique, date_utils, config
...
def route(route=None, **kw):
    ...
    def decorator(f):
        if route:
            ...
            # routing['csrf'] = False
            routing[
                'cors'] = 'Content-Disposition, Set-Cookie, Authorization,Content-Type,Accept-Language,Accept,X-Openerp-Session-Id'
    ...
    def dispatch(self):
        if request.httprequest.method == 'OPTIONS' and request.endpoint and request.endpoint.routing.get('cors'):
            headers = {
                ...
                'Access-Control-Allow-Headers': "Origin, X-Requested-With, X-Openerp-Session-Id, Content-Type, Content-Disposition, Accept, Key, Authorization, Accept-Language, enctype, remember-me, Access-Control-Allow-Credentials",
                'Access-Control-Allow-Origin': request.httprequest.headers.get('origin'),
                'Access-Control-Expose-Headers': 'Content-Disposition, Set-Cookie, Authorization',
            }
...
class OpenERPSession(werkzeug.contrib.sessions.Session):
    ...
    def post_sso_authenticate(self, db, login=None, uid=None):
        """
        Authenticate the current user with the given db, login and
        password. If successful, store the authentication parameters in the
        current session and request.
        ...
        if uid is None:
            ...
            uid = odoo.registry(db)['res.users'].post_sso_authenticate(db, login, env)
        else:
            return None
        self.rotate = True
        self.db = db
...
def session_gc(session_store):
    if random.random() < 0.001:
        ...
        last_week = time.time() - 60 * 60 * 24 * 1
...
class Response(werkzeug.wrappers.Response):
    ...
    def set_default(self, template=None, qcontext=None, uid=None):
        ...
        self.headers.set('Access-Control-Max-Age', "3600")
        self.headers.set("Access-Control-Expose-Headers", "Content-Disposition, Set-Cookie, Authorization")
        self.headers.set('Access-Control-Allow-Headers',
                         "Origin, X-Requested-With, X-Openerp-Session-Id, Content-Type, Content-Disposition, Accept, Key, Authorization, Accept-Language, enctype, remember-me, Access-Control-Allow-Credentials")
    ...
    def get_response(self, httprequest, result, explicit_session):
        ...
        if not explicit_session and hasattr(response, 'set_cookie'):
            if request.httprequest.path != '/web/database/drop' and request.session.db:
                configured_max_age = config.get('session_max_age')
                if configured_max_age:
                    try:
                        ...
                        pass
            response.set_cookie(
                'session_id', httprequest.session.sid, max_age=max_age * 10 * 60 * 60, httponly=True)
        if httprequest.path == '/web/login_new':
            if str(response.data, 'utf-8') != '':
                response.set_cookie(
                    'session_id', httprequest.session.sid, max_age=max_age * 10 * 60 * 60, httponly=True)
                result = json.loads(json.loads(str(response.data, 'utf-8'))['result'])
                result['session_id'] = httprequest.session.sid
                response.data = json.dumps({'result': result, 'status': 200})
        return response
