...
from odoo.tools import config, human_size, ustr, html_escape, odoo
...
class IrAttachment(models.Model):
    ...
    status = fields.Selection(
        ...
        string='Status', context={'status': 'running'}, default='running')
    # for external access
    ...
    def _read_group_allowed_fields(self):
        return ['type', 'company_id', 'res_id', 'create_date', 'create_uid', 'name', 'mimetype', 'id', 'url',
                'res_field', 'res_model']
    ...
    def _search(self, args, offset=0, limit=None, order=None, count=False, access_rights_uid=None):
        ...
        self._cr.execute("""SELECT id, res_model, res_id, public, res_field FROM ir_attachment WHERE id IN %s""",
                         [tuple(ids)])
    ...
    def write(self, vals):
        b = self.check('write', values=vals)
        # remove computed field depending of datas
        for field in ('file_size', 'checksum'):
            vals.pop(field, False)
        if 'mimetype' in vals or 'datas' in vals:
            vals = self._check_contents(vals)
        super(IrAttachment, self).write(vals)

        update_ir = self.env[IrAttachment._name].search([('id', '=', self['id'])])
        url = config['image_host'] + ':' + config['server_port']
        store_fname = update_ir['store_fname']
        uri_path = url + "/images/" + str(store_fname)
        # # query = """ Update public.ir_attachment
        # #             Set uri_path = %s
        # #             Where id = %s
        # #         """
        # # http.request.env.cr.execute(query, (uri_path,self['id']))
        # update_ir.write({'uri_path': uri_path})
        #
        # super(IrAttachment, self).write(vals)
        super(IrAttachment, self).write({'uri_path': uri_path})
    ...
    def create(self, vals_list):
        ...
        for values in vals_list:
            ...
            if 'datas' in values:
                ...
                uri_path = url + "/images/" + str(store_fname)
                values.update({'uri_path': uri_path})
    ...
    def get_attachment_by_key(self, key, extra_domain=None, order=None):
        ...
        #     download_url = '/web/content/' + str(380) + '?download=true'
