...
class Groups(models.Model):
    ...
    active_fleet = fields.Boolean(string='Active fleet',
                           help="Active group for fleet admin add for fleet user.",default=False)
    ...
    def _search(self, args, offset=0, limit=None, order=None, count=False, access_rights_uid=None):
        ...
        return super(Groups, self)._search(args, offset=offset, limit=limit, order=order, count=count,
...
class NotificationChannel(models.Model):
    ...
    name = fields.Selection([('dlp', 'DLP'), ('fleet', 'Fleet'),
                             ('customer', 'Customer')], string='Channel Name',
                            default='dlp', required=True)
    ...
    channel_type = fields.Selection([('manager', 'Manager'), ('employee', 'Employee')], string='Channel type',
...
class Users(models.Model):
    ...
    SELF_READABLE_FIELDS = ['signature', 'company_id', 'login', 'email', 'name', 'image_1920', 'image_1024',
                            'channel_id',
                            'image_512', 'image_256', 'image_128', 'lang', 'tz', 'tz_offset', 'groups_id', 'partner_id',
                            '__last_update', 'action_id']
    channel_id = fields.Many2one('sharevan.channel', string="Channel")
    is_admin = fields.Boolean(default=False)
    channel_role = fields.Char('SSO Role',related='channel_id.channel_code',store=False)
    ...
    login = fields.Char('Username', required=True, help="Used to log into the system")
    ...
    account_fleet_type = fields.Boolean(default=False)
    ...
    _sql_constraints = [
        ('login_key', 'UNIQUE (login)',
         'You can not have two users with the same login ! Check user login name please !')
    ]
    ...
    def _check_login(self):
        if len(self.login.strip().split()) > 1:
            raise ValidationError("Username must not contain whitespace!")
        if not re.match("^[a-zA-Z0-9_]*$", self.login):
            raise ValidationError("Username must not contain any special character")
    ...
    def read_group(self, domain, fields, groupby, offset=0, limit=None, orderby=False, lazy=True):
        ...
        return super(Users, self).read_group(domain, fields, groupby, offset=offset, limit=limit, orderby=orderby,
                                             lazy=lazy)
    ...
    def create(self, vals_list):
        ...
        for user in vals_list:
            ...
            if 'account_fleet_type' in user and user['account_fleet_type'] == True:
                duplicate_query="""
                    select id,active from res_users where login = %s
                """
                self.env.cr.execute(duplicate_query,
                                    (user['login'],))
                duplicate_account = self._cr.dictfetchall()
                if duplicate_account:
                    for acc in duplicate_account:
                        if acc['active']== False:
                            account_update_id.append(acc['id'])
                            update_duplicate_query = """
                                    update res_users set login = %s where id = %s
                                            """
                            self.env.cr.execute(update_duplicate_query,
                                                (user['login']+'_deactivate',acc['id'],))
        users = super(Users, self).create(vals_list)
        for user in users:
            if len(account_update_id) > 0 :
                active = auth_api.active(request.session.access_token, self.env.user.lang[:2], user['login'])
                active_decode = json.loads(active.decode("utf-8"))
                if 'error' in active_decode:
                    raise ValidationError("Account created but failed to active!")
            else:
                name = user['name'].split()
                user_object = {
                    "user_name": user['login'],
                    "first_name": user['name'].replace(name[len(name) - 1], '').strip(),
                    "last_name": name[len(name) - 1],
                }
                user_object_json = json.dumps(user_object)
                print('Json dumps object ---- User : ', user_object_json)


                # async def getResult():
                #     print('Vao trong day: ', request.session.access_token, self.env.user.lang[:2], user_object)
                #     return await  auth_api.create_user(request.session.access_token, self.env.user.lang[:2], user_object)
                # result = getResult()
                result =  auth_api.create_user(request.session.access_token, self.env.user.lang[:2], user_object)
                print('What is result? ', result)
                result_decode = json.loads(result.decode("utf-8"))
                if 'error' in result_decode:
                    if 'message' in result_decode:
                        raise ValidationError(result_decode['message'])
                    raise ValidationError(result_decode['message'])
            ...
            # if partner is global we keep it that way oddo action
    ...
    def reset_password_sso(self, username):
        if not username:
            raise ValidationError('Invalid username!')
        if not request.session.access_token:
            raise AccessDenied()
        authorization = 'Bearer ' + request.session.access_token
        lang = request.session.context['lang'][:2]
        result = auth_api.reset_password(authorization, lang, username)
        result_decode = json.loads(result.decode("utf-8"))
        if 'error' in result_decode:
            raise UserError("Failed to reset password!")
        return True
    ...
    @classmethod
    def post_sso_authenticate(cls, db, login, user_agent_env):
        """
            After login with SSO, get user by username and return
           :param str db: the database on which user is trying to authenticate
           :param str login: username
           :param dict user_agent_env: environment dictionary describing any
               relevant environment attributes
    ...
    def change_password(self, old_passwd, new_passwd):
        ...
        #     return UserError(_("Setting same passwords is not allowed for security reasons!"))
        # if new_passwd:
        #     # use self.env.user here, because it has uid=SUPERUSER_ID
        #     return self.env.user.write({'password': new_passwd})
        # raise UserError(_("Setting empty passwords is not allowed for security reasons!"))

        # Switch to manage user account with sso
        if old_passwd == new_passwd:
            return UserError(_("Setting same passwords is not allowed for security reasons!"))
        if new_passwd:
            # pass old and new password to sso API here
            if not request.session.access_token:
                raise AccessDenied()
            authentication = 'Bearer ' + request.session.access_token
            res = auth_api.change_password(authentication, request.session.context['lang'][:2],
                                           old_passwd, new_passwd, new_passwd)
            res_decode = json.loads(res.decode("utf-8"))
            if 'error' in res_decode:
                if 'message' in res_decode:
                    raise ValidationError(res_decode['message'])
                raise RuntimeError()
            return True
        raise UserError(_("Setting empty passwords is not allowed for security reasons!"))
...
class ChangePasswordWizard(models.TransientModel):
    ...
    def reset_password_button(self):
        username = self.user_ids.user_login
        result = self.env['res.users'].reset_password_sso(username)
        return result
