...
def check_address_contain_special_character(string, field_name):
    if string:
        string_check = re.compile('[@_+!#$%^&*()<>?\|}{~:]')
        if string_check.search(string) is None:
            return True
...
def check_start_date_greater_than_current_date(start_date):
    if start_date:
        today = date.today()
        current_date = datetime.strptime(str(today), '%Y-%m-%d')
        start_date = datetime.strptime(str(start_date), '%Y-%m-%d')
        if start_date < current_date:
            raise ValidationError(_('"'+ str(start_date) + '"' + ' Start date must greater than current date!'))
