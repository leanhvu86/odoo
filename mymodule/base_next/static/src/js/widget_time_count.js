odoo.define('base_next.TimeCount', function (require) {
    "use strict";
    ...
    var FieldMinuteCountUp = fields.FieldChar.extend({
        ...
        _renderReadonly: function () {
            ...
            if (self.record.data && self.record.data.publish_time) {
                var timerPublish = self.record.data.publish_time.toDate()
                var countDownDate = new Date(timerPublish.getTime() + minute * 60000).getTime();
                setInterval(function () {
                        var now = new Date().getTime();
                        //   Find the distance between now and the count down date
                        var distance = countDownDate - now;
                        ...
                        self.$el.text(viewTime);
                    }, 1000
                )
            }
            // self.$el.text("EXPIRED")
            // setInterval(function () {
            //     this.$el.text(minute + ' : ' + sec);
            //     sec--;
            //     if (sec == 0) {
            //         minute--;
            //         sec = 60;
