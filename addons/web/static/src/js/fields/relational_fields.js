    ...
    */
    ...
    var FieldMany2ManyBinaryMultiFiles = AbstractField.extend({
        ...
        events: {
            ...
            'click .icon-left-images': '_onPrev',
            'click .icon-right-images': '_onNext',
            'click .item-image': '_onShowImage',
        ...
        _getFileUrlPath: function (attachment) {
            return attachment.uri_path;
        },
        /**
        ...
        _generatedMetadata: function () {
            ...
            _.each(this.value.data, function (record) {
                ...
                self.metadata[record.id] = {
                    ...
                    uri_path: self._getFileUrlPath(record.data),
                };
            });
        },
        /**
        ...
        _onPrev: function (ev) {
            var id =  Number($(".show-image").attr("index"));
            var length = this.value.data.length;
            var src_img = "";
            var index = 0 ;

            for (var i = 0; i < $(".item-image").length; i++) {
                if (($(".item-image")[i].attributes[1].value) == id){
                    if (i == 0) {
                        src_img = $(".item-image")[i].currentSrc;
                        index =  Number($(".item-image")[i].attributes[1].value);
                        break;
                    }
                    src_img = $(".item-image")[i-1].currentSrc;
                    index =  Number($(".item-image")[i-1].attributes[1].value);
                }
            }


            $(".show-image").attr("src", src_img);
            $(".show-image").attr("index", index);
        ...
        _onShowImage: function (ev) {
            var i = 0;
            var src_img = ""
            var cur_index = Number(ev.currentTarget.attributes[1].value);

            for (i = 0; i < $(".item-image").length; i++) {
                if (($(".item-image")[i].attributes[1].value) == cur_index){
                    src_img = $(".item-image")[i].currentSrc;
                }
            }
            $(".show-image").attr("src", src_img);
            $(".show-image").attr("index", cur_index);
        },
...
//------------------------------------------------------------------------------
    ...
    var FieldSelectionBadge = FieldSelection.extend({
        ...
        _renderEdit: function () {
            ...
            this.$el.empty();
            this.$el.html(qweb.render('FieldSelectionBadge', {
                'values': this.values,
